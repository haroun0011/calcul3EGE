...
async function saveVisitor(name) {
    ...
    const entry = { name: name, time: new Date().toISOString(), average: avg };

    // 1. Cache locally so admin panel works even if offline
    let cache = [];
    try { cache = JSON.parse(localStorage.getItem(LOCAL_CACHE_KEY) || '[]'); } catch(e) {}
    // Update existing record for this session, or add new one
    const idx = cache.findIndex(v => v._sessionName === name);
    if (idx >= 0) {
        cache[idx].average = avg;
    } else {
        entry._sessionName = name;
        cache.push(entry);
    }
...
async function openAdmin() {
    ...
    document.getElementById('adminTbody').innerHTML = '<tr><td colspan="4" style="text-align:center;padding:20px;color:#9ca3af;">جاري التحميل…</td></tr>';
...
function renderAdminTable(visitors) {
    ...
    if (visitors.length === 0) {
        ...
        [...visitors].reverse().forEach((v, i) => {
            ...
            // Format average with color badge
...
/* ============================================================
   ORIGINAL APP CODE (unchanged except calling pushAverageUpdate)
   ============================================================ */
...
function initializeTabs() {
    ...
    // prevent swipe when touching inputs or cards
    if (
...
function calculateOverallAverage(){
    ...
    if(newVal!==null){
        animateValue(overallDisplay,prev,newVal,600);
        // Push average update to sheet after animation settles
